    ...
    return (
        <div
            ...
            <div className="absolute inset-0 opacity-5 pointer-events-none" style={{
                backgroundImage: 'radial-gradient(#a3e635 1px, transparent 1px)', backgroundSize: '20px 20px'
            }}></div>
            ...
            <div className="bg-slate-950/50 p-4 border-t border-slate-800">
                ...
                <div className="flex items-end gap-1 h-12 justify-between">
                    {bars.map((height, i) => (
                        <motion.div key={i} initial={{ height: 0 }} animate={{ height: `${height}%` }} transition={{
                            duration: 0.5,
                            delay: i * 0.05
                        }} className={`flex-1 rounded-t-sm ${i > 10 ? 'bg-lime-500' : 'bg-slate-700'}`}
                        />
                    ))}
                </div>
            </div>
